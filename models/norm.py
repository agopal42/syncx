...
def sqrt_init(shape, dtype=None):
    value = (1 / torch.sqrt(torch.tensor(2.0))) * torch.ones(shape)
    return value
...
def sanitizedInitGet(init):
    if init in ["sqrt_init"]:
        ...
        raise ValueError(f"Initializer type: {init} not supported!")
def sanitizedInitSer(init):
    if init in [sqrt_init]:
        return "sqrt_init"
    else:
        raise ValueError(f"Initializer typeL {init} not supported!")
...
def complex_standardization(input_centred, Vrr, Vii, Vri,
    ...
    input_dim = torch.shape(input_centred)[axis] // 2
    ...
    rolled_input = torch.cat([centred_imag, centred_real], dim=axis)
...
def ComplexBN(input_centred, Vrr, Vii, Vri, beta,
    ...
    if scale:
        ...
        if center:
            broadcast_beta = torch.reshape(beta, broadcast_beta_shape)
...
class ComplexBatchNormalization(nn.Module):
    ...
    def call(self, inputs, training=None):
        ...
        mu = torch.mean(inputs, dim=reduction_axes)
        ...
        if training in {0, False}:
            ...
            def normalize_inference():
                if self.center:
                    inference_centred = inputs - torch.reshape(self.moving_mean, broadcast_mu_shape)
                else:
...
#         # TODO: change to x_norm = x - x.min / (x.max - x.min + eps) * weight + bias
